...
# is active against PDE6D, using the trained PU-Learning MLP
# model (PU_model.keras).
#
# Usage
# -----
...
def load_model(model_path: str):
    """Load the trained Keras PU-Learning model."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Predict PDE6D activity probability from SMILES using the "
                    "trained PU-Learning MLP model (PU_model.keras)."
    )
    ...
    parser.add_argument(
        ...
        help="Output CSV file (used only with -f). Default: predictions.csv"
    )
    parser.add_argument(
        "-m", "--model",
        default="PU_model.keras",
        help="Path to the trained Keras model. Default: PU_model.keras"
    )
    parser.add_argument(
        "--smiles-col",
        default="SMILES",
    ...
    if args.smiles:
        ...
        if fp is None:
            ...
            sys.exit(1)
        proba = model.predict(fp.reshape(1, -1), verbose=0).ravel()[0]
